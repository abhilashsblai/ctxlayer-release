...
[![Release](https://img.shields.io/badge/release-0.2.0a17-blue)](https://github.com/abhilashsblai/ctxlayer-release/releases/tag/v0.2.0a17)
...
`0.2.0a13` introduced the updater bootstrap, `0.2.0a14` repairs legacy-only
project migration, `0.2.0a15` repairs candidate installation from the wheel
cache, `0.2.0a16` repairs migration/rollback compatibility, and `0.2.0a17`
allows durable migration when large-repository indexing must be deferred.
Install `0.2.0a17` once on machines without a working updater.
After that, update from each CTX-enabled project:
...
The updater verifies the signed release manifest and wheel hash, installs an
immutable candidate engine, backs up project state, migrates legacy durable
knowledge and learning into split storage, rebuilds only source-derived index
data, and atomically activates the new engine. Use `ctxlayer update --dry-run`
to verify without cutover or `ctxlayer update --rollback` to restore the prior
engine and project state.
...
See the full release notes for the exact v0.2.0a17 scope and caveats:
[docs/releases/v0.2.0a17.md](docs/releases/v0.2.0a17.md).

## Latest Release

Latest wheel: `ctxlayer-0.2.0a17-py3-none-any.whl`

New build: July 12, 2026 from the Advanced CTX Layer `v0.2.0a17` source tag.
The signed release manifest and `SHA256SUMS` asset contain the authoritative
wheel digest and size generated by the protected release workflow.
...
- Large repositories may defer rebuildable source indexing without rolling
  back verified durable data migration.
- Explicit column-name migration supports legacy/split schema ordering
  differences while preserving row/content parity.
- Stable backup manifests, recoverable rollback failures, and declared-root
  cleanup protect interrupted migrations.
- Canonical wheel staging fixes pip installation from digest-named verified
  caches and unblocks future GitHub-driven upgrades.
- Same-version legacy project repair instead of returning `already_current`
  before required project migration.
- FTS and sqlite-vec virtual indexes are rebuilt without blocking durable-data
  migration, including version-specific tables such as `vec_chunks_128`.
- Signed one-command GitHub wheel updates with immutable engine slots and per-project bindings.
- Verified monolith-to-split migration that preserves durable project knowledge,
  learning, task history, activity, audit, and policy data.
- Automatic rollback, crash recovery, registry provenance, SBOM, release hashes,
  and installed-wheel smoke evidence.
- Provider-backed reasoner routes remain quarantined behind explicit authority,
  promotion policy, and adoption boundaries.
- Live checkpoint reconsideration, experiential lessons, critique evidence,
  reward-scored sandbox proposal lineage, and narrative/world-model telemetry
  are wired into the operational path.
...
- [v0.2.0a17 release notes](docs/releases/v0.2.0a17.md)
- [v0.2.0a16 release notes](docs/releases/v0.2.0a16.md)
- [v0.2.0a15 release notes](docs/releases/v0.2.0a15.md)
- [v0.2.0a14 release notes](docs/releases/v0.2.0a14.md)
- [v0.2.0a13 release notes](docs/releases/v0.2.0a13.md)
- [v0.2.0a12 release notes](docs/releases/v0.2.0a12.md)
