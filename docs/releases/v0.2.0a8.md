...
`9c6d227`. It includes the workflow orchestrator, feature-catalog workbook drift
checks, CI workflow linting, CIE surface expansion, snapshot-aware workflow
resume, multi-agent adapter support, anticipation preview, reliable-enforcement
work, large-DB performance work, memory optimization, deep-GC maintenance, skill
evolution, and write-time semantic guardrails.
